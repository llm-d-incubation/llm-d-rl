...
class AsyncRolloutControllerClient:
    """Async HTTP client for the llm-d-rl rollout controller.

    Same API surface as RolloutControllerClient but uses aiohttp so all
    calls are non-blocking coroutines.  Suitable for use inside an asyncio
    event loop (e.g. verl's OneStepOffRayTrainer).
    """
    ...
    def __init__(self, config: LlmdRolloutConfig | None = None, base_url: str | None = None):
        ...
        self.config = config
        self.base_url = config.controller_url.rstrip("/")
        self.timeout = aiohttp.ClientTimeout(total=config.http_timeout_s)
    ...
    async def wait_for_ready(self, timeout: float = 600) -> dict:
        start = time.time()
        while time.time() - start < timeout:
    ...
    async def get_pool_status(self) -> dict:
        return await self._get("/v1/pool/status")

    # --- Generation ---

    async def generate(self, prompt_token_ids: list[int], max_tokens: int = 32,
                       temperature: float = 0.7, prompt: str | None = None, **kwargs) -> dict:
        body: dict[str, Any] = {
            "prompt_token_ids": prompt_token_ids,
            "return_logprobs": True,
        ...
        if prompt is not None:
            body["prompt"] = prompt
        return await self._post("/v1/generate", body)

    # --- Weight Management ---

    async def init_weight_transfer(self, master_address: str, master_port: int,
                                   world_size: int, backend: str = "nccl") -> dict:
        return await self._post("/v1/weights/init", {
            "backend": backend,
            "master_address": master_address,
            "master_port": master_port,
            "trainer_world_size": world_size,
    ...
    async def update_weights(self, target_version: int,
                             param_names: list[str] | None = None,
                             param_dtypes: list[str] | None = None,
                             param_shapes: list[list[int]] | None = None,
                             pause_mode: str = "keep",
        ...
        if param_names is not None:
            ...
            if self.config.use_packed:
                ...
                body["packed_buffer_size_bytes"] = packed_buffer_size_bytes
                body["packed_num_buffers"] = packed_num_buffers
        return await self._post("/v1/weights/update", body)

    async def update_weights_from_model(self, target_version: int, model) -> dict:
        names, dtypes, shapes = [], [], []
        for name, param in model.named_parameters():
            names.append(name)
            dtypes.append(str(param.dtype).replace("torch.", ""))
            shapes.append(list(param.shape))
        return await self.update_weights(target_version, names, dtypes, shapes)

    async def get_weight_version(self) -> int:
        data = await self._get("/v1/weights/version")
        return data.get("weight_version", 0)

    # --- Engine Lifecycle ---

    async def sleep(self, level: int = 2) -> dict:
        return await self._post("/v1/engines/sleep", {"level": level})

    async def wake_up(self, tags: list[str]) -> dict:
        return await self._post("/v1/engines/wake", {"tags": tags})

    async def pause(self, mode: str = "keep") -> dict:
        return await self._post("/v1/engines/pause", {"mode": mode})

    async def resume(self) -> dict:
        return await self._post("/v1/engines/resume", {})
