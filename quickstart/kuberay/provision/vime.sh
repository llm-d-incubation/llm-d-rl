#!/usr/bin/env bash
# Provision vime on one Ray node.   usage: vime.sh <head|worker>
#
# vime's layout is a single pod with the GPUs on the head, so only "head" does
# real work. It also starts the router stack here, because vime has no in-process
# hook to start EPP from - see llm-d-rl-router.
set -euo pipefail
source "$(dirname "$0")/_common.sh"

ROLE="${1:?usage: vime.sh <head|worker>}"
VIME_REPO="${VIME_REPO:-https://github.com/vllm-project/vime.git}"
VIME_REF="${VIME_REF:-main}"
...
llmd_log "vime provisioning complete on $ROLE"
